...
const productos = require("../models/catalogoProductos");
...
// Para obtener los datos de un producto del catalogo en especifico segun el id
...
// Para obtener los datos de un producto del catalogo segun el numero interno
...
// Para obtener los datos de un producto de catalogo segun la materia prima
router.get("/obtenerPorMP/:materiaPrima", verifyToken ,async (req, res) => {
    const { materiaPrima } = req.params;
    //console.log("buscando")
    await productos
        .findOne({ materiaPrima })
        .then((data) => res.json(data))
        .catch((error) => res.json({ message: error }));
});
